...
export default function PizzaPartyHub() {
  const { unlockedLevels, starRatings } = useGameStore();
  const unlockedLevel = unlockedLevels.pizzaParty;
  const pizzaStars = starRatings.pizzaParty;

  return (
    <div className="zone-pizza flex flex-col items-center min-h-screen relative overflow-y-auto overflow-x-hidden px-4 py-8">
      ...
      >
        ...
        <p className="text-pizza/80 text-lg font-semibold mt-1 tracking-wide">
          Fractions Adventure
        </p>
      </motion.div>
          ...
          return (
            ...
            >
              ...
              {isLocked ? (
                ...
                <Link href={`/pizza-party/${level}`} className="mb-8">
                  <motion.div
                    className={`level-node w-18 h-18 rounded-full flex flex-col items-center justify-center
                      bg-gradient-to-br from-[#ffa502] to-[#ff6348] border-3 border-white/30
                      shadow-[0_0_20px_rgba(255,165,2,0.3)] cursor-pointer`}
