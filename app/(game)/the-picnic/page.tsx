...
export default function PicnicHub() {
  const { unlockedLevels, starRatings } = useGameStore();
  const unlockedLevel = unlockedLevels.picnic;
  const picnicStars = starRatings.picnic;

  return (
    <div className="zone-picnic flex flex-col items-center min-h-screen relative overflow-y-auto overflow-x-hidden px-4 py-8">
      ...
      >
        ...
        <p className="text-picnic/80 text-lg font-semibold mt-1 tracking-wide">
          ...
          return (
            ...
            >
              ...
              {isLocked ? (
                ...
                <Link href={`/the-picnic/${level}`} className="mb-8">
                  <motion.div
                    className={`level-node w-18 h-18 rounded-full flex flex-col items-center justify-center
                      bg-gradient-to-br from-picnic/80 to-picnic border-3 border-white/30
                      shadow-[0_0_20px_rgba(112,161,255,0.3)] cursor-pointer`}
