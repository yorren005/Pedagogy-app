...
export default function GameLevelPage() {
  ...
  const difficulty = zoneDifficulties["storyBuilder"] || 'easy';
  const learnerState = zoneLearnerStates["storyBuilder"] || { skillLevel: 10, recentSuccessStreak: 0, recentFailureStreak: 0 };
  ...
  const generateInternalQuestion = () => {
    let sentence, scrambled, answer, options = [];
  ...
  const sentences = [
    ...
    "The astronaut flew high.",
    "A happy puppy barked.",
    "They played in park.",
    "Reading books is fun.",
    "We finished the story."
  ];
  
  sentence = sentences[(levelNum - 1) % sentences.length];
  const clean = sentence.replace(/[.]/g, '');
  const words = clean.split(' ');
  scrambled = [...words].sort(() => Math.random() - 0.5);
  answer = words.join(' ');
  
  // Create 3 decoy variations
  options = [
    answer,
    [...words].reverse().join(' '),
    [...scrambled].join(' ')
  ].filter((x, idx, self) => self.indexOf(x) === idx).slice(0, 3);
  if (options.length < 3) {
    options.push(words.slice(1).concat(words[0]).join(' '));
  }
  options = options.sort(() => Math.random() - 0.5);
  
  return { equation: "Arrange the words to build a story sentence:", answer, options, scrambled, words };
  };
  ...
  const handleCorrect = () => {
    ...
    setTimeout(() => {
      if (currentRound + 1 >= totalRounds) {
        ...
        addXP(earnedXP);
        setStarRating("storyBuilder", levelNum, starRating.stars);
        unlockNextLevel("storyBuilder", levelNum);
        updateZoneLearnerState("storyBuilder", rlResult.nextState);
        updateZoneDifficulty("storyBuilder", rlResult.nextDifficulty);
  ...
  return (
    <div className="flex-1 w-full relative">
      ...
      <GameShell
        zoneClass="zone-story text-white"
        backHref="/elementary/story-builder"
        title="Story Builder"
        levelNum={levelNum}
          ...
          <div className="w-full max-w-md flex flex-col items-center">
            ...
            <div className="w-full flex flex-col items-center p-6 bg-white/5 border border-white/10 rounded-3xl mb-4">
                  <div className="flex gap-2 flex-wrap justify-center py-4">
                    {problem.scrambled.map((w: string, idx: number) => (
                      <motion.div 
                        key={idx}
                        className="px-4 py-2 bg-purple-600/30 border border-purple-400/50 rounded-xl flex items-center justify-center text-lg font-bold shadow-md cursor-pointer"
      ...
      {gamePhase === "complete" && (
        <LevelComplete
          ...
          equation={problem.equation}
          onNext={() => router.push(`/elementary/story-builder/${levelNum + 1}`)}
          onRetry={() => {
            setCurrentRound(0);
            setGamePhase("playing");
            setTotalErrors(0);
