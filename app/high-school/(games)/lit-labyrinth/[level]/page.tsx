...
export default function GameLevelPage() {
  ...
  const difficulty = zoneDifficulties["litLabyrinth"] || 'easy';
  const learnerState = zoneLearnerStates["litLabyrinth"] || { skillLevel: 10, recentSuccessStreak: 0, recentFailureStreak: 0 };
  ...
  const generateInternalQuestion = () => {
    let passage, answer, options, equation;
  
  const bank = [
    { p: "'The autumn leaves fell like tears from the weeping trees.'", a: "Melancholic", q: "Identify the tone of this passage:" },
    { p: "'A green light blinked mockingly across the dark bay, forever out of reach.'", a: "Symbolism", q: "Identify the literary device used:" },
    { p: "'The clock ticked like a beating heart, counting down the last seconds.'", a: "Simile", q: "Identify the literary device:" },
    { p: "'The giant sat in his dark mansion, lonely and regretful of his past.'", a: "Characterization", q: "Identify the literary focus:" }
  ];
  const choice = bank[Math.floor(Math.random() * bank.length)];
  passage = choice.p;
  answer = choice.a;
  equation = choice.q + " " + choice.p;
  options = ["Melancholic", "Symbolism", "Simile", "Characterization", "Theme"].filter((x, idx, self) => self.indexOf(x) === idx).slice(0, 3);
  if (!options.includes(answer)) options[0] = answer;
  options = options.sort(() => Math.random() - 0.5);
  return { equation, answer, options, passage };
  };
  ...
  const handleCorrect = () => {
    ...
    setTimeout(() => {
      if (currentRound + 1 >= totalRounds) {
        ...
        addXP(earnedXP);
        setStarRating("litLabyrinth", levelNum, starRating.stars);
        unlockNextLevel("litLabyrinth", levelNum);
        updateZoneLearnerState("litLabyrinth", rlResult.nextState);
        updateZoneDifficulty("litLabyrinth", rlResult.nextDifficulty);
  ...
  return (
    <div className="flex-1 w-full relative">
      ...
      <GameShell
        ...
        backHref="/high-school/lit-labyrinth"
        title="Lit Labyrinth"
        levelNum={levelNum}
        ...
        <WrongShake trigger={showWrong}>
          <div className="w-full max-w-md flex flex-col items-center">
            ...
            <div className="w-full flex flex-col items-center p-6 bg-white/5 border border-white/10 rounded-3xl mb-4">
                  <div className="p-4 bg-purple-950/20 border border-purple-500/20 rounded-2xl max-w-sm text-center">
                    <p className="text-sm font-semibold text-white/90 italic">{problem.passage}</p>
                  </div>
                </div>
      ...
      {gamePhase === "complete" && (
        <LevelComplete
          ...
          equation={problem.equation}
          onNext={() => router.push(`/high-school/lit-labyrinth/${levelNum + 1}`)}
          onRetry={() => {
            setCurrentRound(0);
            setGamePhase("playing");
            setTotalErrors(0);
