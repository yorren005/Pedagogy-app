...
export default function GameLevelPage() {
  ...
  const difficulty = zoneDifficulties["matrixMaze"] || 'easy';
  const learnerState = zoneLearnerStates["matrixMaze"] || { skillLevel: 10, recentSuccessStreak: 0, recentFailureStreak: 0 };
  ...
  const generateInternalQuestion = () => {
    let equation, answer, options, matrixStr;
  
  const list = [
    { m: "[[1, 2], [3, 4]]", q: "Find determinant of Matrix A = [[1, 2], [3, 4]]", a: "-2" },
    { m: "[[2, 5], [1, 3]]", q: "Find determinant of Matrix B = [[2, 5], [1, 3]]", a: "1" },
    { m: "[[3, 1], [4, 2]]", q: "Find determinant of Matrix C = [[3, 1], [4, 2]]", a: "2" }
  ];
  const choice = list[Math.floor(Math.random() * list.length)];
  matrixStr = choice.m;
  equation = choice.q;
  answer = choice.a;
  options = [answer, "0", "4", "-2", "1", "2"].filter((x, idx, self) => self.indexOf(x) === idx).slice(0, 3);
  if (!options.includes(answer)) options[0] = answer;
  options = options.sort(() => Math.random() - 0.5);
  return { equation, answer, options, matrixStr };
  };
  ...
  const handleCorrect = () => {
    ...
    setTimeout(() => {
      if (currentRound + 1 >= totalRounds) {
        ...
        addXP(earnedXP);
        setStarRating("matrixMaze", levelNum, starRating.stars);
        unlockNextLevel("matrixMaze", levelNum);
        updateZoneLearnerState("matrixMaze", rlResult.nextState);
        updateZoneDifficulty("matrixMaze", rlResult.nextDifficulty);
  ...
  return (
    <div className="flex-1 w-full relative">
      ...
      <GameShell
        ...
        backHref="/high-school/matrix-maze"
        title="Matrix Maze"
        levelNum={levelNum}
        ...
        <WrongShake trigger={showWrong}>
          <div className="w-full max-w-md flex flex-col items-center">
            ...
            <div className="w-full flex flex-col items-center p-6 bg-white/5 border border-white/10 rounded-3xl mb-4">
                  <div className="font-mono text-xl p-4 bg-purple-950/20 border border-purple-500/20 rounded-xl flex items-center justify-center gap-1.5 shadow-md">
                    <span>A = </span>
                    <span className="text-purple-300 font-extrabold">{problem.matrixStr}</span>
                  </div>
                </div>
      ...
      {gamePhase === "complete" && (
        <LevelComplete
          ...
          equation={problem.equation}
          onNext={() => router.push(`/high-school/matrix-maze/${levelNum + 1}`)}
          onRetry={() => {
            setCurrentRound(0);
            setGamePhase("playing");
            setTotalErrors(0);
