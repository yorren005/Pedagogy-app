...
export default function GameLevelPage() {
  ...
  const difficulty = zoneDifficulties["trigTower"] || 'easy';
  const learnerState = zoneLearnerStates["trigTower"] || { skillLevel: 10, recentSuccessStreak: 0, recentFailureStreak: 0 };
  ...
  const generateInternalQuestion = () => {
    let equation, answer, options, angle;
  if (levelNum <= 10) {
    const list = [
      { q: "sin(30°)", a: "1/2" },
      { q: "cos(60°)", a: "1/2" },
  ...
  options = [answer, "0", "1/2", "√3/2", "-1"].filter((x, idx, self) => self.indexOf(x) === idx).slice(0, 3);
  ...
  const handleCorrect = () => {
    ...
    setTimeout(() => {
      if (currentRound + 1 >= totalRounds) {
        ...
        addXP(earnedXP);
        setStarRating("trigTower", levelNum, starRating.stars);
        unlockNextLevel("trigTower", levelNum);
        updateZoneLearnerState("trigTower", rlResult.nextState);
        updateZoneDifficulty("trigTower", rlResult.nextDifficulty);
  ...
  return (
    <div className="flex-1 w-full relative">
      ...
      <GameShell
        ...
        backHref="/high-school/trig-tower"
        title="Trig Tower"
        levelNum={levelNum}
      ...
      {gamePhase === "complete" && (
        <LevelComplete
          ...
          equation={problem.equation}
          onNext={() => router.push(`/high-school/trig-tower/${levelNum + 1}`)}
          onRetry={() => {
            setCurrentRound(0);
            setGamePhase("playing");
            setTotalErrors(0);
