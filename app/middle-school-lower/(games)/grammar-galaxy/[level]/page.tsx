...
export default function GameLevelPage() {
  ...
  const difficulty = zoneDifficulties["grammarGalaxy"] || 'easy';
  const learnerState = zoneLearnerStates["grammarGalaxy"] || { skillLevel: 10, recentSuccessStreak: 0, recentFailureStreak: 0 };
  ...
  const generateInternalQuestion = () => {
    let sentence, targetWord, answer, options, equation;
  
  const bank = [
    { s: "The astronaut flew to the stars.", w: "flew", a: "verb" },
    { s: "The red rocket was very fast.", w: "rocket", a: "noun" },
    { s: "Jupiter is an extremely giant planet.", w: "giant", a: "adjective" },
    { s: "He quickly repaired the spacesuit.", w: "quickly", a: "adverb" },
    { s: "She looked at the bright moon.", w: "bright", a: "adjective" },
    { s: "Gravity pulls things down.", w: "pulls", a: "verb" },
    { s: "Stars shine bright in the night sky.", w: "sky", a: "noun" },
    { s: "The silent universe holds many secrets.", w: "silent", a: "adjective" }
  ];
  ...
  equation = `Identify the part of speech of "${targetWord}"`;
  options = ["noun", "verb", "adjective", "adverb"].sort(() => Math.random() - 0.5);
  return { equation, answer, options, sentence, targetWord };
  };
  ...
  const handleCorrect = () => {
    ...
    setTimeout(() => {
      if (currentRound + 1 >= totalRounds) {
        ...
        addXP(earnedXP);
        setStarRating("grammarGalaxy", levelNum, starRating.stars);
        unlockNextLevel("grammarGalaxy", levelNum);
        updateZoneLearnerState("grammarGalaxy", rlResult.nextState);
        updateZoneDifficulty("grammarGalaxy", rlResult.nextDifficulty);
  ...
  return (
    <div className="flex-1 w-full relative">
      ...
      <GameShell
        ...
        backHref="/middle-school-lower/grammar-galaxy"
        ...
        <WrongShake trigger={showWrong}>
          <div className="w-full max-w-md flex flex-col items-center">
            ...
            <div className="w-full flex flex-col items-center p-6 bg-white/5 border border-white/10 rounded-3xl mb-4">
                  <div className="p-4 bg-black/20 rounded-2xl border border-white/5 text-center max-w-sm mb-4">
                    <p className="text-lg text-white font-medium">
                      {problem.sentence.split(' ').map((word: string, i: number) => {
                        const cleaned = word.replace(/[.,]/g, '');
                        const isTarget = cleaned.toLowerCase() === problem.targetWord.toLowerCase();
                        return (
                          <span key={i} className={isTarget ? "text-pink-400 font-extrabold underline mx-1" : "mx-1"}>
                            {word}
                          </span>
                        );
                      })}
      ...
      {gamePhase === "complete" && (
        <LevelComplete
          ...
          equation={problem.equation}
          onNext={() => router.push(`/middle-school-lower/grammar-galaxy/${levelNum + 1}`)}
          onRetry={() => {
            setCurrentRound(0);
            setGamePhase("playing");
            setTotalErrors(0);
