...
export default function GameLevelPage() {
  ...
  const difficulty = zoneDifficulties["vocabVault"] || 'easy';
  const learnerState = zoneLearnerStates["vocabVault"] || { skillLevel: 10, recentSuccessStreak: 0, recentFailureStreak: 0 };
  ...
  const generateInternalQuestion = () => {
    let equation, answer, options, clue;
  
  const bank = [
    { q: "What is the meaning of the root 'bio'?", a: "life", c: "used in biology, biography" },
    { q: "What does the prefix 'anti' mean?", a: "against", c: "used in antisocial, antibody" },
    { q: "What does the root 'chrono' relate to?", a: "time", c: "used in chronological, chronometer" },
    { q: "What does the suffix 'logy' mean?", a: "study of", c: "used in biology, psychology" },
    { q: "What is the meaning of the prefix 'bene'?", a: "good", c: "used in beneficial, benevolent" },
    { q: "What does the root 'geo' mean?", a: "earth", c: "used in geography, geology" }
  ];
  ...
  answer = choice.a;
  clue = choice.c;
  options = [answer, "self", "death", "study of", "light", "against"].filter((x, idx, self) => self.indexOf(x) === idx).slice(0, 4);
  if (!options.includes(answer)) options[0] = answer;
  options = options.sort(() => Math.random() - 0.5);
  return { equation, answer, options, clue };
  };
  ...
  const handleCorrect = () => {
    ...
    setTimeout(() => {
      if (currentRound + 1 >= totalRounds) {
        ...
        addXP(earnedXP);
        setStarRating("vocabVault", levelNum, starRating.stars);
        unlockNextLevel("vocabVault", levelNum);
        updateZoneLearnerState("vocabVault", rlResult.nextState);
        updateZoneDifficulty("vocabVault", rlResult.nextDifficulty);
  ...
  return (
    <div className="flex-1 w-full relative">
      ...
      <GameShell
        ...
        backHref="/middle-school-lower/vocab-vault"
        title="Vocab Vault"
        levelNum={levelNum}
        ...
        <WrongShake trigger={showWrong}>
          <div className="w-full max-w-md flex flex-col items-center">
            ...
            <div className="w-full flex flex-col items-center p-6 bg-white/5 border border-white/10 rounded-3xl mb-4">
                  <div className="flex gap-2 items-center bg-yellow-500/10 border border-yellow-500/20 rounded-xl p-3 max-w-sm mb-4">
                    <span className="text-xl">💡</span>
                    <p className="text-xs text-yellow-300 font-semibold leading-relaxed">Hint: {problem.clue}</p>
                  </div>
                </div>
      ...
      {gamePhase === "complete" && (
        <LevelComplete
          ...
          equation={problem.equation}
          onNext={() => router.push(`/middle-school-lower/vocab-vault/${levelNum + 1}`)}
          onRetry={() => {
            setCurrentRound(0);
            setGamePhase("playing");
            setTotalErrors(0);
