...
export default function GameLevelPage() {
  ...
  const difficulty = zoneDifficulties["essayEngine"] || 'easy';
  const learnerState = zoneLearnerStates["essayEngine"] || { skillLevel: 10, recentSuccessStreak: 0, recentFailureStreak: 0 };
  ...
  const generateInternalQuestion = () => {
    let equation, answer, options, hint;
  
  const bank = [
    { q: "Where does the thesis statement typically belong in a 5-paragraph essay?", a: "End of introduction", h: "Sets up the road map for the entire essay." },
    { q: "Which part of an essay hooks the reader's attention?", a: "Introduction hook", h: "Located right at the beginning." },
    { q: "What is the primary purpose of a body paragraph topic sentence?", a: "State main point of paragraph", h: "Controls the paragraph's focus." },
    { q: "What should a conclusion paragraph do?", a: "Restate thesis & summarize", h: "Wraps up and leaves final thoughts." }
  ];
  const choice = bank[Math.floor(Math.random() * bank.length)];
  equation = choice.q;
  answer = choice.a;
  hint = choice.h;
  options = [answer, "Beginning of body paragraph", "End of conclusion", "In the bibliography"].filter((x, idx, self) => self.indexOf(x) === idx).slice(0, 3);
  if (!options.includes(answer)) options[0] = answer;
  options = options.sort(() => Math.random() - 0.5);
  return { equation, answer, options, hint };
  };
  ...
  const handleCorrect = () => {
    ...
    setTimeout(() => {
      if (currentRound + 1 >= totalRounds) {
        ...
        addXP(earnedXP);
        setStarRating("essayEngine", levelNum, starRating.stars);
        unlockNextLevel("essayEngine", levelNum);
        updateZoneLearnerState("essayEngine", rlResult.nextState);
        updateZoneDifficulty("essayEngine", rlResult.nextDifficulty);
  ...
  return (
    <div className="flex-1 w-full relative">
      ...
      <GameShell
        ...
        backHref="/middle-school-upper/essay-engine"
        title="Essay Engine"
        levelNum={levelNum}
        ...
        <WrongShake trigger={showWrong}>
          <div className="w-full max-w-md flex flex-col items-center">
            ...
            <div className="w-full flex flex-col items-center p-6 bg-white/5 border border-white/10 rounded-3xl mb-4">
                  <div className="flex gap-2 items-center bg-amber-500/10 border border-amber-500/20 rounded-xl p-3 max-w-sm mb-4">
                    <span className="text-xl">💡</span>
                    <p className="text-xs text-amber-300 font-semibold leading-relaxed">Hint: {problem.hint}</p>
                  </div>
                </div>
      ...
      {gamePhase === "complete" && (
        <LevelComplete
          ...
          equation={problem.equation}
          onNext={() => router.push(`/middle-school-upper/essay-engine/${levelNum + 1}`)}
          onRetry={() => {
            setCurrentRound(0);
            setGamePhase("playing");
            setTotalErrors(0);
