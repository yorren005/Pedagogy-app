...
export default function GameLevelPage() {
  ...
  const difficulty = zoneDifficulties["functionForge"] || 'easy';
  const learnerState = zoneLearnerStates["functionForge"] || { skillLevel: 10, recentSuccessStreak: 0, recentFailureStreak: 0 };
  ...
  const generateInternalQuestion = () => {
    let a, b, x, answer, options, equation;
  if (levelNum <= 7) {
    // Linear function evaluation: f(x) = ax + b
    a = Math.floor(Math.random() * 4) + 2;
    b = Math.floor(Math.random() * 6) + 1;
    x = Math.floor(Math.random() * 4) + 1;
    answer = a * x + b;
    equation = `If f(x) = ${a}x + ${b}, find f(${x})`;
  } else {
    // Quadratic function evaluation: f(x) = x^2 + ax + b
    a = Math.floor(Math.random() * 3) + 1;
    b = Math.floor(Math.random() * 4) + 1;
    x = Math.floor(Math.random() * 3) + 1;
    answer = x * x + a * x + b;
    equation = `If f(x) = x² + ${a}x + ${b}, find f(${x})`;
  }
  ...
  const handleCorrect = () => {
    ...
    setTimeout(() => {
      if (currentRound + 1 >= totalRounds) {
        ...
        addXP(earnedXP);
        setStarRating("functionForge", levelNum, starRating.stars);
        unlockNextLevel("functionForge", levelNum);
        updateZoneLearnerState("functionForge", rlResult.nextState);
        updateZoneDifficulty("functionForge", rlResult.nextDifficulty);
  ...
  return (
    <div className="flex-1 w-full relative">
      ...
      <GameShell
        ...
        backHref="/middle-school-upper/function-forge"
      ...
      {gamePhase === "complete" && (
        <LevelComplete
          ...
          equation={problem.equation}
          onNext={() => router.push(`/middle-school-upper/function-forge/${levelNum + 1}`)}
          onRetry={() => {
            setCurrentRound(0);
            setGamePhase("playing");
            setTotalErrors(0);
