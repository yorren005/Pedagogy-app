...
export default function GameLevelPage() {
  ...
  const difficulty = zoneDifficulties["probabilityPinball"] || 'easy';
  const learnerState = zoneLearnerStates["probabilityPinball"] || { skillLevel: 10, recentSuccessStreak: 0, recentFailureStreak: 0 };
  ...
  const generateInternalQuestion = () => {
    let equation, answer, options, probText;
  if (levelNum <= 7) {
    // Single dice roll
    const diceSide = Math.floor(Math.random() * 3) + 3; // e.g. rolling greater than 3,4,5
    answer = (6 - diceSide) + "/6";
    equation = `Find probability of rolling greater than ${diceSide} on a 6-sided die.`;
  } else {
    // Coin flips
    answer = "1/4";
    equation = "Find probability of getting 2 heads in a row when flipping a fair coin twice.";
  }
  options = [answer, "1/2", "1/3", "1/6", "3/4"].filter((x, idx, self) => self.indexOf(x) === idx).slice(0, 3);
  ...
  const handleCorrect = () => {
    ...
    setTimeout(() => {
      if (currentRound + 1 >= totalRounds) {
        ...
        addXP(earnedXP);
        setStarRating("probabilityPinball", levelNum, starRating.stars);
        unlockNextLevel("probabilityPinball", levelNum);
        updateZoneLearnerState("probabilityPinball", rlResult.nextState);
        updateZoneDifficulty("probabilityPinball", rlResult.nextDifficulty);
  ...
  return (
    <div className="flex-1 w-full relative">
      ...
      <GameShell
        ...
        backHref="/middle-school-upper/probability-pinball"
        title="Probability Pinball"
        levelNum={levelNum}
      ...
      {gamePhase === "complete" && (
        <LevelComplete
          ...
          equation={problem.equation}
          onNext={() => router.push(`/middle-school-upper/probability-pinball/${levelNum + 1}`)}
          onRetry={() => {
            setCurrentRound(0);
            setGamePhase("playing");
            setTotalErrors(0);
