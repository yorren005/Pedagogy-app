...
export default function GameLevelPage() {
  ...
  const difficulty = zoneDifficulties["rhetoricArena"] || 'easy';
  const learnerState = zoneLearnerStates["rhetoricArena"] || { skillLevel: 10, recentSuccessStreak: 0, recentFailureStreak: 0 };
  ...
  const bank = [
    { p: "'As a doctor with 20 years of experience, I recommend this treatment.'", a: "Ethos", q: "Identify the rhetorical appeal:" },
    { p: "'If we do not act now, innocent children will suffer.'", a: "Pathos", q: "Identify the rhetorical appeal:" },
    { p: "'Studies show that 95% of users saw improvement in 2 weeks.'", a: "Logos", q: "Identify the rhetorical appeal:" },
    { p: "'The classroom was a zoo during the fire drill.'", a: "Metaphor", q: "Identify the figurative language:" }
  ];
  ...
  options = ["Ethos", "Pathos", "Logos", "Metaphor", "Simile"].filter((x, idx, self) => self.indexOf(x) === idx).slice(0, 3);
  ...
  const handleCorrect = () => {
    ...
    setTimeout(() => {
      if (currentRound + 1 >= totalRounds) {
        ...
        addXP(earnedXP);
        setStarRating("rhetoricArena", levelNum, starRating.stars);
        unlockNextLevel("rhetoricArena", levelNum);
        updateZoneLearnerState("rhetoricArena", rlResult.nextState);
        updateZoneDifficulty("rhetoricArena", rlResult.nextDifficulty);
  ...
  return (
    <div className="flex-1 w-full relative">
      ...
      <GameShell
        ...
        backHref="/middle-school-upper/rhetoric-arena"
        title="Rhetoric Arena"
        levelNum={levelNum}
      ...
      {gamePhase === "complete" && (
        <LevelComplete
          ...
          equation={problem.equation}
          onNext={() => router.push(`/middle-school-upper/rhetoric-arena/${levelNum + 1}`)}
          onRetry={() => {
            setCurrentRound(0);
            setGamePhase("playing");
            setTotalErrors(0);
