...
export default function GameLevelPage() {
  ...
  const difficulty = zoneDifficulties["abstractArena"] || 'easy';
  const learnerState = zoneLearnerStates["abstractArena"] || { skillLevel: 10, recentSuccessStreak: 0, recentFailureStreak: 0 };
  ...
  const generateInternalQuestion = () => {
    let equation, answer, options, groupProps;
  
  const list = [
    { q: "Which property requires 'a * b is in G' for all a, b in G?", a: "Closure", p: "G is closed under operation *" },
    { q: "Which property requires 'a * (b * c) = (a * b) * c'?", a: "Associativity", p: "Order of operations doesn't matter" },
    { q: "Which property requires 'a * e = a' for all a?", a: "Identity", p: "Element e does not change others" }
  ];
  const choice = list[Math.floor(Math.random() * list.length)];
  equation = choice.q;
  answer = choice.a;
  groupProps = choice.p;
  options = ["Closure", "Associativity", "Identity", "Inverse"].sort(() => Math.random() - 0.5);
  return { equation, answer, options, groupProps };
  };
  ...
  const handleCorrect = () => {
    ...
    setTimeout(() => {
      if (currentRound + 1 >= totalRounds) {
        ...
        addXP(earnedXP);
        setStarRating("abstractArena", levelNum, starRating.stars);
        unlockNextLevel("abstractArena", levelNum);
        updateZoneLearnerState("abstractArena", rlResult.nextState);
        updateZoneDifficulty("abstractArena", rlResult.nextDifficulty);
  ...
  return (
    <div className="flex-1 w-full relative">
      ...
      <GameShell
        ...
        backHref="/university/abstract-arena"
        title="Abstract Arena"
        levelNum={levelNum}
        ...
        <WrongShake trigger={showWrong}>
          <div className="w-full max-w-md flex flex-col items-center">
            ...
            <div className="w-full flex flex-col items-center p-6 bg-white/5 border border-white/10 rounded-3xl mb-4">
                  <div className="bg-orange-500/10 border border-orange-500/20 rounded-xl p-3 text-center max-w-sm">
                    <p className="text-xs text-orange-300 font-semibold">Group Axiom: {problem.groupProps}</p>
                  </div>
                </div>
      ...
      {gamePhase === "complete" && (
        <LevelComplete
          ...
          equation={problem.equation}
          onNext={() => router.push(`/university/abstract-arena/${levelNum + 1}`)}
          onRetry={() => {
            setCurrentRound(0);
            setGamePhase("playing");
            setTotalErrors(0);
