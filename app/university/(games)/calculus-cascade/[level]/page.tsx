...
export default function GameLevelPage() {
  ...
  const difficulty = zoneDifficulties["calculusCascade"] || 'easy';
  const learnerState = zoneLearnerStates["calculusCascade"] || { skillLevel: 10, recentSuccessStreak: 0, recentFailureStreak: 0 };
  ...
  if (levelNum <= 12) {
    ...
    const list = [
      { q: "Does Σ (1/n) converge?", a: "Diverges" },
      { q: "Does Σ (1/n²) converge?", a: "Converges" },
      { q: "Does Σ (1/2^n) converge?", a: "Converges" }
    ];
    const choice = list[Math.floor(Math.random() * list.length)];
    equation = choice.q;
    answer = choice.a;
  }
  options = [answer, "x³/3 + C", "1/x", "2e^(2x)", "Converges", "Diverges", "0"].filter((x, idx, self) => self.indexOf(x) === idx).slice(0, 3);
  ...
  const handleCorrect = () => {
    ...
    setTimeout(() => {
      if (currentRound + 1 >= totalRounds) {
        ...
        addXP(earnedXP);
        setStarRating("calculusCascade", levelNum, starRating.stars);
        unlockNextLevel("calculusCascade", levelNum);
        updateZoneLearnerState("calculusCascade", rlResult.nextState);
        updateZoneDifficulty("calculusCascade", rlResult.nextDifficulty);
  ...
  return (
    <div className="flex-1 w-full relative">
      ...
      <GameShell
        ...
        backHref="/university/calculus-cascade"
        title="Calculus Cascade"
        levelNum={levelNum}
          ...
          <div className="w-full max-w-md flex flex-col items-center">
            ...
            <div className="w-full flex flex-col items-center p-6 bg-white/5 border border-white/10 rounded-3xl mb-4 relative overflow-hidden">
                  <div className="absolute top-2 left-4 text-xs text-white/40 font-bold uppercase tracking-wider">
                    Calculus Stream Cascade
                  </div>
                  <div className="w-32 h-20 relative flex items-center justify-center mt-2">
                    <div className="absolute inset-0 bg-sky-500/10 border border-sky-400/20 rounded-xl flex items-center justify-center">
                      <span className="text-5xl text-sky-400 font-extrabold animate-pulse">∫</span>
                    </div>
                  </div>
                </div>
      ...
      {gamePhase === "complete" && (
        <LevelComplete
          ...
          equation={problem.equation}
          onNext={() => router.push(`/university/calculus-cascade/${levelNum + 1}`)}
          onRetry={() => {
            setCurrentRound(0);
            setGamePhase("playing");
            setTotalErrors(0);
