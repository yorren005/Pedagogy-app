...
export default function GameLevelPage() {
  ...
  const difficulty = zoneDifficulties["criticalLens"] || 'easy';
  const learnerState = zoneLearnerStates["criticalLens"] || { skillLevel: 10, recentSuccessStreak: 0, recentFailureStreak: 0 };
  ...
  const bank = [
    { p: "Analyzing class conflict, power dynamics, and socioeconomic status in a novel:", a: "Marxism", q: "Identify the critical lens:" },
    { p: "Examining patriarchal structures, gender roles, and female agency in a text:", a: "Feminism", q: "Identify the critical lens:" },
    { p: "Exploring the subconscious, dream imagery, and desires of characters:", a: "Psychoanalysis", q: "Identify the critical lens:" },
    { p: "Exploring identity, hegemony, and colonizer-colonized dynamics in post-colonial literature:", a: "Post-Colonialism", q: "Identify the critical lens:" }
  ];
  const choice = bank[Math.floor(Math.random() * bank.length)];
  passage = choice.p;
  answer = choice.a;
  equation = choice.q + " " + choice.p;
  options = ["Marxism", "Feminism", "Psychoanalysis", "Post-Colonialism"].sort(() => Math.random() - 0.5);
  ...
  const handleCorrect = () => {
    ...
    setTimeout(() => {
      if (currentRound + 1 >= totalRounds) {
        ...
        addXP(earnedXP);
        setStarRating("criticalLens", levelNum, starRating.stars);
        unlockNextLevel("criticalLens", levelNum);
        updateZoneLearnerState("criticalLens", rlResult.nextState);
        updateZoneDifficulty("criticalLens", rlResult.nextDifficulty);
  ...
  return (
    <div className="flex-1 w-full relative">
      ...
      <GameShell
        ...
        backHref="/university/critical-lens"
        title="Critical Lens"
        levelNum={levelNum}
        ...
        <WrongShake trigger={showWrong}>
          <div className="w-full max-w-md flex flex-col items-center">
            ...
            <div className="w-full flex flex-col items-center p-6 bg-white/5 border border-white/10 rounded-3xl mb-4">
                  <div className="p-4 bg-teal-950/20 border border-teal-500/20 rounded-2xl max-w-sm text-center">
      ...
      {gamePhase === "complete" && (
        <LevelComplete
          ...
          equation={problem.equation}
          onNext={() => router.push(`/university/critical-lens/${levelNum + 1}`)}
          onRetry={() => {
            setCurrentRound(0);
            setGamePhase("playing");
            setTotalErrors(0);
