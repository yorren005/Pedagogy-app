...
export default function GameLevelPage() {
  ...
  const difficulty = zoneDifficulties["proofArchitect"] || 'easy';
  const learnerState = zoneLearnerStates["proofArchitect"] || { skillLevel: 10, recentSuccessStreak: 0, recentFailureStreak: 0 };
  ...
  const generateInternalQuestion = () => {
    let equation, answer, options, proofMethod;
  
  const list = [
    { q: "To prove P(n) for all integers, what is the standard method?", a: "Mathematical Induction", m: "Base case + inductive step." },
    { q: "To prove A implies B, you assume not B and derive not A. This is:", a: "Proof by Contrapositive", m: "Equivalent to direct implication." },
    { q: "To prove A, you assume not A and find a logical contradiction. This is:", a: "Proof by Contradiction", m: "Reductio ad absurdum." }
  ];
  const choice = list[Math.floor(Math.random() * list.length)];
  equation = choice.q;
  answer = choice.a;
  proofMethod = choice.m;
  options = [answer, "Mathematical Induction", "Proof by Contrapositive", "Proof by Contradiction", "Direct Proof"].filter((x, idx, self) => self.indexOf(x) === idx).slice(0, 3);
  if (!options.includes(answer)) options[0] = answer;
  options = options.sort(() => Math.random() - 0.5);
  return { equation, answer, options, proofMethod };
  };
  ...
  const handleCorrect = () => {
    ...
    setTimeout(() => {
      if (currentRound + 1 >= totalRounds) {
        ...
        addXP(earnedXP);
        setStarRating("proofArchitect", levelNum, starRating.stars);
        unlockNextLevel("proofArchitect", levelNum);
        updateZoneLearnerState("proofArchitect", rlResult.nextState);
        updateZoneDifficulty("proofArchitect", rlResult.nextDifficulty);
  ...
  return (
    <div className="flex-1 w-full relative">
      ...
      <GameShell
        ...
        backHref="/university/proof-architect"
        ...
        <WrongShake trigger={showWrong}>
          <div className="w-full max-w-md flex flex-col items-center">
            ...
            <div className="w-full flex flex-col items-center p-6 bg-white/5 border border-white/10 rounded-3xl mb-4">
                  <div className="bg-purple-500/10 border border-purple-500/20 rounded-xl p-3 text-center max-w-sm">
                    <p className="text-xs text-purple-300 font-semibold">Architectural Step: {problem.proofMethod}</p>
                  </div>
                </div>
      ...
      {gamePhase === "complete" && (
        <LevelComplete
          ...
          equation={problem.equation}
          onNext={() => router.push(`/university/proof-architect/${levelNum + 1}`)}
          onRetry={() => {
            setCurrentRound(0);
            setGamePhase("playing");
            setTotalErrors(0);
