...
export default function GameLevelPage() {
  ...
  const difficulty = zoneDifficulties["thesisForge"] || 'easy';
  const learnerState = zoneLearnerStates["thesisForge"] || { skillLevel: 10, recentSuccessStreak: 0, recentFailureStreak: 0 };
  ...
  const generateInternalQuestion = () => {
    let equation, answer, options, citeStyle;
  
  const list = [
    { q: "Which format uses (Author, Year) in-text citation?", a: "APA", c: "American Psychological Association." },
    { q: "Which format uses (Author Page) in-text citation?", a: "MLA", c: "Modern Language Association." },
    { q: "Which format uses numbered footnotes?", a: "Chicago", c: "Chicago Manual of Style." }
  ];
  const choice = list[Math.floor(Math.random() * list.length)];
  equation = choice.q;
  answer = choice.a;
  citeStyle = choice.c;
  options = ["APA", "MLA", "Chicago", "Harvard"].sort(() => Math.random() - 0.5);
  return { equation, answer, options, citeStyle };
  };
  ...
  const handleCorrect = () => {
    ...
    setTimeout(() => {
      if (currentRound + 1 >= totalRounds) {
        ...
        addXP(earnedXP);
        setStarRating("thesisForge", levelNum, starRating.stars);
        unlockNextLevel("thesisForge", levelNum);
        updateZoneLearnerState("thesisForge", rlResult.nextState);
        updateZoneDifficulty("thesisForge", rlResult.nextDifficulty);
  ...
  return (
    <div className="flex-1 w-full relative">
      ...
      <GameShell
        ...
        backHref="/university/thesis-forge"
        title="Thesis Forge"
        levelNum={levelNum}
        ...
        <WrongShake trigger={showWrong}>
          <div className="w-full max-w-md flex flex-col items-center">
            ...
            <div className="w-full flex flex-col items-center p-6 bg-white/5 border border-white/10 rounded-3xl mb-4">
                  <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-xl p-3 text-center max-w-sm">
                    <p className="text-xs text-yellow-300 font-semibold">Style Guide: {problem.citeStyle}</p>
                  </div>
                </div>
      ...
      {gamePhase === "complete" && (
        <LevelComplete
          ...
          equation={problem.equation}
          onNext={() => router.push(`/university/thesis-forge/${levelNum + 1}`)}
          onRetry={() => {
            setCurrentRound(0);
            setGamePhase("playing");
            setTotalErrors(0);
